...
##################################################################################
################## Time and Interaction Box & Mean Plots #########################
##################################################################################

all_time_map_data <- read.csv("../datasets-participants-time/all_time_map.csv")
all_time_calendar_data <- read.csv("../datasets-participants-time/all_time_calendar.csv")
conditions_time_map_data <- read.csv("../datasets-participants-time/conditions_time_map.csv")
conditions_time_calendar_data <- read.csv("../datasets-participants-time/conditions_time_calendar.csv")
clusters_time_map_data <- read.csv("../datasets-participants-time/clusters_time_map.csv")
clusters_time_calendar_data <- read.csv("../datasets-participants-time/clusters_time_calendar.csv")
conditions_clusters_time_map_data <- read.csv("../datasets-participants-time/conditions_clusters_time_map.csv")
conditions_clusters_time_calendar_data <- read.csv("../datasets-participants-time/conditions_clusters_time_calendar.csv")

all_interactions_map_data <- read.csv("../datasets-participants-interactions/all_interactions_map.csv")
all_interactions_calendar_data <- read.csv("../datasets-participants-interactions/all_interactions_calendar.csv")
conditions_interactions_map_data <- read.csv("../datasets-participants-interactions/conditions_interactions_map.csv")
conditions_interactions_calendar_data <- read.csv("../datasets-participants-interactions/conditions_interactions_calendar.csv")
clusters_interactions_map_data <- read.csv("../datasets-participants-interactions/clusters_interactions_map.csv")
clusters_interactions_calendar_data <- read.csv("../datasets-participants-interactions/clusters_interactions_calendar.csv")
conditions_clusters_interactions_map_data <- read.csv("../datasets-participants-interactions/conditions_clusters_interactions_map.csv")
conditions_clusters_interactions_calendar_data <- read.csv("../datasets-participants-interactions/conditions_clusters_interactions_calendar.csv")
...
all_interactions_map_df <- data.frame(all_interactions_map_data)
all_interactions_calendar_df <- data.frame(all_interactions_calendar_data)
conditions_interactions_map_df <- data.frame(conditions_interactions_map_data)
conditions_interactions_calendar_df <- data.frame(conditions_interactions_calendar_data)
clusters_interactions_map_df <- data.frame(clusters_interactions_map_data)
clusters_interactions_calendar_df <- data.frame(clusters_interactions_calendar_data)
conditions_clusters_interactions_map_df <- data.frame(conditions_clusters_interactions_map_data)
conditions_clusters_interactions_calendar_df <- data.frame(conditions_clusters_interactions_calendar_data)

# All Map Time Box Plot
...
# All Calendar Time Box Plot
ggplot(all_time_calendar_df, aes(y=total_time_calendar)) + 
  geom_boxplot(color="#bad9d0",outlier.colour="red", outlier.shape=8,
               outlier.size=4) + theme_classic() + ylab('Total Time Calendar')

# All Map Interactions Box Plot
ggplot(all_interactions_map_df, aes(y=total_interactions_map)) + 
  geom_boxplot(color="#bad9d0",outlier.colour="red", outlier.shape=8,
               outlier.size=4) + theme_classic() + ylab('Total Interations Map') 

# All Calendar Time Box Plot
ggplot(all_interactions_calendar_df, aes(y=total_interactions_calendar)) + 
  geom_boxplot(color="#bad9d0",outlier.colour="red", outlier.shape=8,
               outlier.size=4) + theme_classic() + ylab('Total Interactions Calendar')

pd <- position_dodge(0.1) # move them .05 to the left and right
conditions_time_map_df_summarized <- summarySE(conditions_time_map_df, measurevar="total_time_map", groupvars=c("condition"))
conditions_time_calendar_df_summarized <- summarySE(conditions_time_calendar_df, measurevar="total_time_calendar", groupvars=c("condition"))
clusters_time_map_df_summarized <- summarySE(clusters_time_map_df, measurevar="total_time_map", groupvars=c("Cluster"))
clusters_time_calendar_df_summarized <- summarySE(clusters_time_calendar_df, measurevar="total_time_calendar", groupvars=c("Cluster"))
conditions_clusters_time_map_df_summarized <- summarySE(conditions_clusters_time_map_df, measurevar="total_time_map", groupvars=c("condition","Cluster"))
conditions_clusters_time_calendar_df_summarized <- summarySE(conditions_clusters_time_calendar_df, measurevar="total_time_calendar", groupvars=c("condition","Cluster"))

conditions_interactions_map_df_summarized <- summarySE(conditions_interactions_map_df, measurevar="total_interactions_map", groupvars=c("condition"))
conditions_interactions_calendar_df_summarized <- summarySE(conditions_interactions_calendar_df, measurevar="total_interactions_calendar", groupvars=c("condition"))
clusters_interactions_map_df_summarized <- summarySE(clusters_interactions_map_df, measurevar="total_interactions_map", groupvars=c("Cluster"))
clusters_interactions_calendar_df_summarized <- summarySE(clusters_interactions_calendar_df, measurevar="total_interactions_calendar", groupvars=c("Cluster"))
conditions_clusters_interactions_map_df_summarized <- summarySE(conditions_clusters_interactions_map_df, measurevar="total_interactions_map", groupvars=c("condition","Cluster"))
conditions_clusters_interactions_calendar_df_summarized <- summarySE(conditions_clusters_interactions_calendar_df, measurevar="total_interactions_calendar", groupvars=c("condition","Cluster"))

# Condition Map Time Mean Plot
ggplot(conditions_time_map_df_summarized, aes(x=condition, y=total_time_map)) + 
  geom_errorbar(aes(ymin=total_time_map-ci, ymax=total_time_map+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Time Map')

# Condition Calendar Time Mean Plot
ggplot(conditions_time_calendar_df_summarized, aes(x=condition, y=total_time_calendar)) + 
  geom_errorbar(aes(ymin=total_time_calendar-ci, ymax=total_time_calendar+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Time Calendar')

# Cluster Map Time Mean Plot
ggplot(clusters_time_map_df_summarized, aes(x=Cluster, y=total_time_map)) + 
  geom_errorbar(aes(ymin=total_time_map-ci, ymax=total_time_map+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Cluster') + ylab('Total Time Map')

# Cluster Calendar Time Mean Plot
ggplot(clusters_time_calendar_df_summarized, aes(x=Cluster, y=total_time_calendar)) + 
  geom_errorbar(aes(ymin=total_time_calendar-ci, ymax=total_time_calendar+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Cluster') + ylab('Total Time Calendar')

# Condition and Cluster Map Time Mean Plot
ggplot(conditions_clusters_time_map_df_summarized, aes(x=condition, y=total_time_map, colour=Cluster)) + 
  geom_errorbar(aes(ymin=total_time_map-ci, ymax=total_time_map+ci), width=.1, position=pd) +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Time Map')+ scale_color_manual(values=c("#36ebab","#1e7557"))

# Condition and Cluster Calendar Time Mean Plot
ggplot(conditions_clusters_time_calendar_df_summarized, aes(x=condition, y=total_time_calendar, colour=Cluster)) + 
  geom_errorbar(aes(ymin=total_time_calendar-ci, ymax=total_time_calendar+ci), width=.1, position=pd) +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Time Calendar') + scale_color_manual(values=c("#36ebab","#1e7557"))

###############################################################################################################

# Condition Map Interactions Mean Plot
ggplot(conditions_interactions_map_df_summarized, aes(x=condition, y=total_interactions_map)) + 
  geom_errorbar(aes(ymin=total_interactions_map-ci, ymax=total_interactions_map+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Interactions Map')

# Condition Calendar Interactions Mean Plot
ggplot(conditions_interactions_calendar_df_summarized, aes(x=condition, y=total_interactions_calendar)) + 
  geom_errorbar(aes(ymin=total_interactions_calendar-ci, ymax=total_interactions_calendar+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Interactions Calendar')

# Cluster Map Interactions Mean Plot
ggplot(clusters_interactions_map_df_summarized, aes(x=Cluster, y=total_interactions_map)) + 
  geom_errorbar(aes(ymin=total_interactions_map-ci, ymax=total_interactions_map+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Cluster') + ylab('Total Interactions Map')

# Cluster Calendar Interactions Mean Plot
ggplot(clusters_interactions_calendar_df_summarized, aes(x=Cluster, y=total_interactions_calendar)) + 
  geom_errorbar(aes(ymin=total_interactions_calendar-ci, ymax=total_interactions_calendar+ci), width=.1, position=pd,colour="#969696") +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Cluster') + ylab('Total Interactions Calendar')

# Condition and Cluster Map Interactions Mean Plot
ggplot(conditions_clusters_interactions_map_df_summarized, aes(x=condition, y=total_interactions_map, colour=Cluster)) + 
  geom_errorbar(aes(ymin=total_interactions_map-ci, ymax=total_interactions_map+ci), width=.1, position=pd) +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Interactions Map') + scale_color_manual(values=c("#36ebab","#1e7557"))

# Condition and Cluster Calendar Interactions Mean Plot
ggplot(conditions_clusters_interactions_calendar_df_summarized, aes(x=condition, y=total_interactions_calendar, colour=Cluster)) + 
  geom_errorbar(aes(ymin=total_interactions_calendar-ci, ymax=total_interactions_calendar+ci), width=.1, position=pd) +
  geom_line(position=pd) +
  geom_point(position=pd) +xlab('Condition') + ylab('Total Interactions Calendar') + scale_color_manual(values=c("#36ebab","#1e7557"))

##################################################################################
############## Locus Dimensions Distribution Box Plots ###########################
##################################################################################

all_dimensions_data <- read.csv("../datasets-participants-dimensions/all_dimensions.csv")
conditions_dimensions_data <- read.csv("../datasets-participants-dimensions/conditions_dimensions.csv")
clusters_dimensions_data <- read.csv("../datasets-participants-dimensions/clusters_dimensions.csv")
conditions_clusters_dimensions_data <- read.csv("../datasets-participants-dimensions/conditions_clusters_dimensions.csv")
...
# Condition Powerful Others Distribution
...
# Condition Cluster Powerful Others Distribution
...
# Condition Cluster Chance Distribution
...
##################################################################################
##### Demographic Distribution (Gender, Age, Education, Glasses) Bar Charts ######
##################################################################################
...
all_glasses_data <- read.csv("../datasets-participants-demographics/all_glasses.csv")
...
conditions_glasses_data <- read.csv("../datasets-participants-demographics/conditions_glasses.csv")
...
clusters_glasses_data <- read.csv("../datasets-participants-demographics/clusters_glasses.csv")
conditions_clusters_distribution_data <- read.csv("../datasets-participants-demographics/conditions_clusters_distribution.csv")
all_gender_df <- data.frame(all_gender_data)
all_age_df <- data.frame(all_age_data)
all_education_df <- data.frame(all_education_data)
all_glasses_df <- data.frame(all_glasses_data)
...
conditions_glasses_df <- data.frame(conditions_glasses_data)
...
clusters_glasses_df <- data.frame(clusters_glasses_data)
conditions_clusters_distribution_df <- data.frame(conditions_clusters_distribution_data)

# Participants Group Distribution
...
ggplot(conditions_clusters_distribution_df, aes(fill=cluster, y=total, x=condition, label=ifelse(total>1,paste0(sprintf("%d", total),""),""))) + geom_bar(position="stack",width=0.8, stat="identity") + geom_text(data=subset(conditions_clusters_distribution_df,total != 0),colour= '#ffffff',size = 8, position = position_stack(vjust = 0.5)) + theme_bw()+ theme(legend.position = "right",legend.title=element_text(size=18, face='bold'),legend.text = element_text(size=15),axis.text.x=element_text(size=15),axis.title.x=element_text(face='bold',size=18,margin = margin(t = 20, r = 0, b = 0, l = 0)),axis.ticks.x=element_blank(),axis.title.y=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(),panel.border = element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank())  + scale_fill_manual(name='Cluster',values=c("#36ebab","#1e7557"),limits = c("Cluster 1", "Cluster 2"))+  xlab('Condition') 

# All Demographics Distribution
...
ggplot(all_glasses_df, aes(as.factor(1),fill=glasses, y=total, label=ifelse(total>1,paste0(sprintf("%d", total),""),""))) + geom_col(position="stack", width=0.4) + geom_text(vjust="inward",hjust="inward",data=subset(all_glasses_df,total != 0),colour= '#ffffff',size = 8, position = position_stack(vjust = 0.5)) + theme_bw()+ theme(legend.position = "right",legend.title=element_text(size=18, face='bold'),legend.text = element_text(size=15),axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank(),axis.title.y=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(),panel.border = element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank())  + scale_fill_manual(name='Vision',values=c("#a7db81","#91d162"),limits = c("Glasses", "No Glasses"))

# Condition Demographics Participants
...
ggplot(conditions_glasses_df, aes(fill=glasses, y=total, x=condition, label=ifelse(total>1,paste0(sprintf("%d", total),""),""))) + geom_bar(position="stack",width=0.8, stat="identity") + geom_text(data=subset(conditions_glasses_df,total != 0),colour= '#ffffff',size = 8, position = position_stack(vjust = 0.5)) + theme_bw()+ theme(legend.position = "none",legend.title=element_text(size=18, face='bold'),legend.text = element_text(size=15),axis.text.x=element_text(size=15),axis.title.x=element_text(face='bold',size=18,margin = margin(t = 20, r = 0, b = 0, l = 0)),axis.ticks.x=element_blank(),axis.title.y=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(),panel.border = element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank())  + scale_fill_manual(name='Vision',values=c("#a7db81","#91d162"),limits = c("Glasses", "No Glasses"))+  xlab('Condition') 
...
ggplot(clusters_glasses_df, aes(fill=glasses, y=total, x=cluster, label=ifelse(total>1,paste0(sprintf("%d", total),""),""))) + geom_bar(position="stack",width=0.8, stat="identity") + geom_text(data=subset(clusters_glasses_df,total != 0),colour= '#ffffff',size = 8, position = position_stack(vjust = 0.5)) + theme_bw()+ theme(legend.position = "none",legend.title=element_text(size=18, face='bold'),legend.text = element_text(size=15),axis.text.x=element_text(size=15),axis.title.x=element_text(face='bold',size=18,margin = margin(t = 20, r = 0, b = 0, l = 0)),axis.ticks.x=element_blank(),axis.title.y=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(),panel.border = element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank())  + scale_fill_manual(name='Vision',values=c("#a7db81","#91d162"),limits = c("Glasses", "No Glasses"))+  xlab('Cluster') 

##################################################################################
### Path of Interactions Top 5 Interactions for each network (Bar Charts) ########
##################################################################################
...
# Calendar Anchor Cluster 1 Network
...
# Calendar Anchor Cluster 2 Network
